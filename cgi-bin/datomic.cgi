...
sub printForm {
  ...
  print qq(<textarea rows="8" cols="100"  name="restQuery" id="restQuery">$restQuery</textarea>);
...
#   print qq(URL $url URL<br/>);
  ...
  print qq(REST $restResult RESULT<br/>);		# put results here for tazendra form to parse out of
  $restResult =~ s/^\[+//;
  $restResult =~ s/\]+$//;
